...
enum class ThoughtKind:std::uint8_t {Notice,Interpret,Recall,Forecast,Compare,Intent,Question,Reply,Stale,Pause,Resume,Count};
enum class Origin:std::uint8_t {Observed,Reported,Inferred,Imagined};
enum class Operation:std::uint8_t {None,Interpret,Recall,Forecast,Compare,Commit,Recognize,Reply,SocialReply,SocialObserve};
