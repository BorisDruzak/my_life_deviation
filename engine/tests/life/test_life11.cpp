...
TEST("life11",waiting_projects_and_parallel_channels_survive_restore){auto a=adult_pair();CHECK(a.conversation_for_test(1,2));a.edit_for_test().autonomy=false;a.run_seconds(10);const auto path=(std::filesystem::temp_directory_path()/"life11-resume.save").string();a.save(path);auto b=World::load(path);a.run_seconds(30);b.run_seconds(30);CHECK(a.hash()==b.hash());std::filesystem::remove(path);}
...
TEST("life11",an_accepted_joint_activity_survives_save_restore){auto a=adult_pair();CHECK(a.conversation_for_test(1,2));a.propose_social_for_test(1,Interaction::PartnerIntimacy,2);await_acceptance(a,Interaction::PartnerIntimacy);a.edit_for_test().autonomy=false;a.run_seconds(300);const auto path=(std::filesystem::temp_directory_path()/"life11-joint.save").string();a.save(path);auto b=World::load(path);a.run_seconds(1800);b.run_seconds(1800);CHECK(a.hash()==b.hash());CHECK(a.state().life.intimacies==1);std::filesystem::remove(path);}
TEST("life11",payroll_and_goals_survive_mid_shift_restore){auto a=World::generate(7,8);a.configure_life_projects();a.run_seconds(16*3600);const auto path=(std::filesystem::temp_directory_path()/"life11-payroll.save").string();a.save(path);auto b=World::load(path);a.run_seconds(2*3600);b.run_seconds(2*3600);CHECK(a.hash()==b.hash());CHECK(a.state().life.organizations[0].payroll.size()==8);std::filesystem::remove(path);}
