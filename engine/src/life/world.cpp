...
void World::physical_until(Actor& a,Tick now){
    ...
    community_account(a,elapsed);civil_physical(a,elapsed);
        ...
        if(s.life.enabled&&a.conversation.id&&conscious){
            const auto& b=s.actors[a.conversation.partner-1];
            if(conversation_together(a,b,a.conversation.id)){
                const auto fit=combine_resources(activity_resources(a.action.method,moving(a),input.asleep),activity_resources(Method::Talk));
        ...
        const std::array<bool,3> critical{a.body.energy<.1,a.body.water<.1,a.body.sleep>.95};
        for(unsigned j=0;j<3;++j){if(critical[j]){
            if(a.critical_current[j]==0)++a.critical_episodes[j];
            a.critical_by_need[j]+=seconds;a.critical_current[j]+=seconds;
            a.critical_longest[j]=std::max(a.critical_longest[j],a.critical_current[j]);
        }else a.critical_current[j]=0;}
...
void World::process_boundary(){
    ...
    if(s.now>=s.next_supply){
        ...
        at(s,3).food+=food;at(s,4).water+=water;
        if(s.adaptive_life){for(const auto& office:s.hiring){at(s,office.place).water+=n*6;s.ledger.supplied_water+=n*6;}}
        s.ledger.supplied_food+=food;s.ledger.supplied_water+=water;++s.ledger.supplies;s.next_supply+=86400000;
    ...
    for(const auto& [actor,decision]:commands){auto& a=s.actors.at(actor-1);if(!a.alive)continue;
        ...
        if(decision.method==Method::Social)start_social(a,decision);
            ...
            if(!life_start_conversation(a,decision)){
                if(auto* p=a.mind.projects.find(decision.project))a.mind.projects.observe(p->id,StepOutcome::Unavailable,s.next_id++,s.now);
            }
        }else {if(a.action.phase!=Phase::Idle)cancel(a);start(a,decision);}
        if(a.conversation.id){const auto* e=social_event(a.mind.social.active_event);
            if(e&&e->kind==Interaction::PartnerIntimacy&&e->phase==SocialPhase::Accepted&&decision.method!=Method::Social)life_stop_conversation(a,false);}
        a.cog.captured_mind=a.mind.version;a.cog.captured_situation=a.cog.situation_version;
    }
    life_conversation_tick(); // Evaluate joint compatibility after the entire same-time command batch.
