...
bool talking_together(const Actor& a,const Actor& b,std::uint64_t parent){
    return a.alive&&b.alive&&a.place==b.place&&a.action.phase==Phase::Running&&b.action.phase==Phase::Running&&
        a.action.method==Method::Talk&&b.action.method==Method::Talk&&a.action.id==parent&&b.action.id==parent&&
        a.action.partner==b.id&&b.action.partner==a.id;
}
}
...
void World::social_known_person(Actor& a,Id id,std::uint64_t source){
    ...
    if(const auto* r=a.mind.relation(id)){p.closeness=r->familiarity;p.competence=r->trust[1].expectation();}
...
void World::populate_social_view(const Actor& a,PersonalView& v)const{
    ...
    target.in_conversation=a.action.method==Method::Talk&&a.action.phase==Phase::Running;
    if(target.in_conversation){target.partner=a.action.partner;target.parent=a.action.id;}
    ...
    for(Id id:ids)if(const auto* p=m.person(id)){out.people.push_back(*p);target.perceived.push_back(*p);}
...
void World::start_social(Actor& a,const Decision& d){
    ...
    if(!talking_together(a,b,a.action.id)||!needs_joint_consent(d.interaction))return;
    ...
    SocialEvent e;e.id=state_.next_id++;e.parent=a.action.id;e.initiator=a.id;e.receiver=b.id;e.object=d.object;
    e.kind=d.interaction;e.proposed_at=state_.now;e.ends=state_.now+30000;
    ...
    message.stage=SocialStage::Offer;message.at=state_.now;message.public_context=e.public_b;
...
void World::social_answer(Actor& b,const InteractionObservation& offer,bool accept,const SocialEvaluation& evaluation){
    ...
    auto message=[&](Actor& recipient,SocialStage stage,SocialReason reason,bool initiated){
        InteractionObservation o;o.event=e->id;o.parent=e->parent;o.other=initiated?b.id:a.id;o.object=e->object;o.kind=e->kind;o.stage=stage;o.reason=reason;o.initiated=initiated;o.at=state_.now;o.public_context=initiated?e->public_a:e->public_b;social_deliver(recipient,o);
    };
    if(!accept){
        e->phase=SocialPhase::Declined;
    ...
    auto response=[&](const Actor& who,const Actor& other){double primary=who.social_traits.pleasure[std::size_t(e->kind)];
        if(e->kind==Interaction::RomanticTouch){primary*=who.social_traits.attraction[unsigned(other.mind.social.gender)];if(state_.community.enabled)primary*=attraction(other.appearance,who.social_traits.preference);}
...
void World::stop_social(Actor& actor){
    ...
    e->phase=SocialPhase::Cancelled;if(e->reason==SocialReason::None)e->reason=SocialReason::Withdrawn;++state_.social.cancelled;
    a.mind.social.active_event=b.mind.social.active_event=0;
    for(Actor* person:{&a,&b}){InteractionObservation o;o.event=e->id;o.parent=e->parent;o.other=person==&a?b.id:a.id;o.object=e->object;o.kind=e->kind;o.stage=SocialStage::Cancelled;o.reason=e->reason;o.at=state_.now;o.initiated=person==&a;o.public_context=person==&a?e->public_a:e->public_b;
...
void World::process_social(){
    ...
    for(auto id:active){auto* e=social_event(id);if(!e)throw std::logic_error("missing active social event");
        ...
        e->phase=SocialPhase::Completed;++state_.social.completed[std::size_t(e->kind)];
        a.mind.social.active_event=b.mind.social.active_event=0;
        for(Actor* who:{&a,&b}){
            InteractionObservation o;o.event=e->id;o.parent=e->parent;o.other=who==&a?b.id:a.id;o.object=e->object;o.kind=e->kind;o.stage=SocialStage::Completed;o.initiated=who==&a;o.public_context=who==&a?e->public_a:e->public_b;o.at=state_.now;
...
void World::seed_social(){
    ...
    for(auto& a:s.actors){auto& m=a.mind.social;
        ...
        for(std::size_t k=0;k<interaction_count;++k){auto& b=m.methods[k];
            b.mastery=.85;b.expected_pleasure=a.social_traits.pleasure[k];b.expected_acceptance=.65;b.confidence=.7;b.source=s.next_id++;
        }
