...
std::string baseline_catalogue_hash(){
    std::string bytes="LIFE-0.9.0;BEHAVIOR-0.3;COG-0.4;balance-cpp-0.9.0-social1;graph-seconds;expected-sanctions;";
    for(std::size_t i=0;i<std::size_t(Method::Visit);++i){bytes+=names[i];bytes+=':';bytes+=std::to_string(specs[i].seconds);bytes+=':';bytes+=std::to_string(std::bit_cast<std::uint64_t>(specs[i].load));bytes+=':';bytes+=std::to_string(std::bit_cast<std::uint64_t>(specs[i].pleasure));bytes+=';';}
    return hex_sha256(bytes);
}
...
std::vector<PlanOption> Planner::ideas(const PersonalView& v,const Seed& random){
    ...
    if(v.life.enabled){
        ...
        if(urgent){
            // Keep ALL known bodily remedies. A second critical signal (fear,
...
Decision Planner::forecast(const PersonalView& v,const PlanOption& idea){
    ...
    if(v.life.enabled&&idea.project){
        ...
        // discounted atomic payoff, then enter the chosen value at t=0 once.
        const double atomic_now=expected*std::exp(-cog04::cfg::discount_per_hour*hours);
        ledger.impulse(1,0,0,std::max(atomic_now,project_forecast(v,idea)));
    }else ledger.impulse(1,0,hours,expected);
