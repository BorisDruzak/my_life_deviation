...
void World::validate()const{
    const auto& s=state_;validate_social_world();
    auto ensure=[](bool condition,const std::string& what){if(!condition)throw std::runtime_error("world invariant: "+what);};
    ensure(s.format=="LIFE-0.10.0"&&(s.balance=="balance-cpp-0.9.0-social1"||s.balance=="community-0.10-test2"),"unsupported format or balance");
...
std::string World::summary_json()const{
    ...
    out<<"{\"version\":\"0.10.0\",\"balance\":\""<<s.balance<<"\",\"catalogue\":\""<<s.random.catalog<<"\",\"seed\":"<<s.random.value<<",\"scenario\":\""<<s.scenario<<"\",\"seconds\":"<<s.now/1000<<",\"population\":"<<s.actors.size()<<",\"alive\":"<<alive<<",\"hash\":\""<<hash()<<"\",\"actions\":{";
