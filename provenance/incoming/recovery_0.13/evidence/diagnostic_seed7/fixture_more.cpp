...
int main(int argc,char**argv){try{
 ...
 auto w=World::generate(seed,16);w.configure_recovery();
 ...
 for(unsigned i=0;i<hours;++i){w.run_seconds(3600);const auto& a=w.state().actors.at(who-1);
  ...
  snapshots<<"],\"sense_food\":"<<w.personal_view(who).need[0]<<",\"snapshot_food\":"<<a.cog.snapshot.need[0]<<",\"food_known\":"<<a.mind.knowledge.get(100)<<",\"food_mean\":"<<a.cog.needs[0].mean<<",\"food_truth\":"<<unsigned(a.cog.needs[0].status)<<",\"band_food\":"<<a.cog.last_signal_band[0]<<"}\n";snapshots.flush();if(!a.alive)break;
 }
 w.save(prefix+".save");std::cout<<w.self_report_json()<<'\n';
