...
void World::begin_at_destination(Actor& a){
    ...
    if(state_.community.enabled){
        ...
        if(state_.life.enabled&&x.method==Method::Work){
            if(a.place!=a.employment.workplace||!a.employment.in_shift(s.now))x.blocked=true;
            if(x.blocked)x.end=s.now+1000; // A late arrival is an observed failure, never an event in the past.
            else x.end=std::min(x.end,(s.now/86400000)*86400000+a.employment.shift_end);
        }
        if(state_.life.enabled&&x.method==Method::Rest)a.employment.break_until=s.now+900000;
    ...
    if(x.method==Method::Talk&&s.life.enabled){
        Decision d;d.method=Method::Talk;d.partner=x.partner;d.place=a.place;d.project=x.project;d.project_step=x.project_step;
        if(!life_start_conversation(a,d)){fail(a,"contact_not_available");}return;
    }
...
void World::cancel(Actor& a){
    ...
    finish_window(a);
    if(state_.community.enabled&&!state_.life.enabled&&a.action.method==Method::Work&&a.action.phase==Phase::Running&&!a.action.blocked){
...
void World::complete(Actor& a){
    ...
    else if(method==Method::Work&&s.life.enabled){employment_credit(a);}
