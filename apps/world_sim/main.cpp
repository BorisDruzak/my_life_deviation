...
namespace {
  ...
  world_sim validate --scenario game/scenarios/household.json [--data game/rules]
  world_sim run --scenario game/scenarios/household.json --minutes 1440 --out out/day
  world_sim resume --snapshot out/day/state.json --scenario game/scenarios/household.json
                   --minutes 1440 --out out/next
  world_sim view --snapshot out/day/state.json --actor n0 [--data game/rules]
...
int main(int argc, char * * argv) {
    try {
        ...
        auto config = Config::load(setting("--data", "game/rules"));
