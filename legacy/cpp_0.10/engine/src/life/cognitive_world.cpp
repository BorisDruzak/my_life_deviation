...
void World::update_senses(Actor& a){
 ...
 for(Id id:visible){
  ...
  if(c.focus_kind==TopicKind::External&&c.focus_person==id&&state_.now>=c.rebuild_until)
...
void World::cognition_tick(Actor& a){
 ...
 if(a.mind.social.enabled){
  ...
  }else if(a.action.method==Method::Talk&&a.action.phase==Phase::Running&&!a.mind.social.active_event){
   const double need=std::max(.5,cues[5]);
   const double pr=cog04::priority(need,.25,.3,.8,0,.5);
   input.push_back({(2ull<<48)+a.action.id,TopicKind::Thought,5,a.action.partner,pr,.25,now,now+31000,c.situation_version,a.action.id,false});
  }
...
void World::cognition_complete(Actor& a,const Decision* forecast){
 ...
 ++c.completed_ops;Thought t;t.started=started;t.metric=c.focus_metric;t.person=c.focus_person;t.basis=c.focus_basis;t.status=Truth::Confirmed;t.confidence=.8;
 ...
 if(op==Operation::Interpret){
  ...
  for(std::size_t j=0;j<metric_count;++j)c.snapshot.need[j]=c.needs[j].status==Truth::Confirmed?c.needs[j].mean:0;
 ...
 if(op==Operation::Commit){
  ...
  const bool nested=d.method==Method::Social&&a.action.method==Method::Talk&&a.action.phase==Phase::Running;
  const bool use_item=d.method==Method::UseObject;
  const bool change=nested||a.action.phase==Phase::Idle||(use_item&&a.action.method==Method::Talk&&a.action.phase==Phase::Running)||(interruptible&&d.method!=Method::Idle&&d.method!=a.action.method&&(emergency||c.snapshot.need[c.focus_metric]>.75));
  ...
  t.kind=ThoughtKind::Intent;t.method=d.method;t.value=d.score;t.basis=c.best_thought;t.detail=change?"команда_исполнителю":"текущее_действие_сохранено";emit_thought(a,t);++metrics_.decisions;
  if(change){
  ...
  if(state_.community.enabled&&c.focus_kind==TopicKind::Internal&&c.focus_metric<9&&d.method==Method::Idle&&c.cursor>0&&!c.best_thought){
   ...
   c.no_continuation_until[c.focus_metric]=state_.now+60000;
