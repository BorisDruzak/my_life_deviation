...
void World::community_publish(Actor& actor,Information fact){
    ...
    for(auto& other:s.actors){if(other.id==actor.id||!other.alive||other.place!=actor.place||other.action.phase==Phase::Travel)continue;
        ...
        o.information=fact;o.information.cited_source=other.id;o.information_present=true;
...
void World::community_finish(Actor& a,Method method){
    ...
    if(method==Method::Work){
        ...
        if(tier>a.economy.job){a.economy.job=tier;++a.economy.promotions;}
        a.economy.wage_known=p.wage;f.kind=NewsKind::Employment;f.other=p.id;f.material=.2+.2*tier;f.importance=.5;
    }else if(method==Method::Study){
        unsigned target=1;while(target<4&&a.mind.knowledge.get(job_catalogue()[target].skill)>=.6)++target;
        if(target<4){for(auto& [id,g]:a.mind.knowledge.mastery)if(id==job_catalogue()[target].skill){const double alpha=-std::expm1(-.5*a.mind.cognition.learnability*.5);g=unit(g+alpha*(1-g));}}
