...
enum class Interaction:std::uint8_t {
    FriendlyTouch,RomanticTouch,Compliment,AskInfo,ShowItem,BorrowItem,ReturnItem,ClaimItem,UseItem,ShareNews,DiscussTopic,Count
};
...
struct PersonBelief {
    Id id=0;Gender gender=Gender::Unknown;double closeness=.5,competence=.5;
    std::uint64_t source=0;Appearance appearance;bool appearance_known=false,known_kin=false;
    template<class A>void fields(A& a){a(id,gender,closeness,competence,source,appearance,appearance_known,known_kin);}
};
...
struct InteractionObservation {
    ...
    template<class A>void fields(A& a){a(delivery,event,parent,other,object,kind,stage,reason,public_context,initiated,pleasure,primary_pleasure,approval,dose,pleasure_observed,approval_observed,at,item,item_present,lesson,lesson_kind,lesson_present,information,information_present);}
};
struct SocialMemory {
...
struct SocialView {
    ...
    template<class A>void fields(A& a){a(enabled,in_conversation,occupied,self,partner,place,now,parent,known_audience,memory,perceived,need_social,need_leisure);}
};
...
struct SocialEvent {
    ...
    template<class A>void fields(A& a){a(id,parent,initiator,receiver,object,kind,phase,reason,proposed_at,answered_at,ends,public_a,public_b,primary_a,primary_b,pleasure_a,pleasure_b,approval_a,approval_b,response,information,information_present,listeners,listener_parents);}
};
