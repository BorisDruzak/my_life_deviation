...
namespace npc {
    ...
    void World::remember(const Id & id, Evidence e) {
        ...
        auto found = std::find_if(a.beliefs.begin(), a.beliefs.end(),[&](const Evidence & x) { return x.id == e.id; });
    ...
    Json data, bool owner_only, const Id & root, Json secret) {
        ...
        for (const auto &[id, x] : state_.actors) {
            ...
            if (! participant) {
                ...
                if (! get < bool >(at(config_.parameters, "perception"), "perfect_observation") && math::keyed_sample(state_.seed, e.uid, id, "sight") >= probability) continue;
            }
            ...
            if (! participant) for (auto key : {"mode", "agreement", "price", "from", "to", "amount", "count"}) public_data.as_object().erase(key);
            e.projections[id] = Object{{"id", e.uid}, {"time", e.time}, {"type", type == "unauthorized_transfer" ? "item_moved" : type},
                {"participants", encode(actors)}, {"objects", encode(objects)}, {"data", public_data}};
        }
    ...
    void World::deliver_projections(std::size_t begin) {
        for (std::size_t n = begin; n < state_.ledger.size(); ++ n) {
            ...
            for (const auto &[id, p] : e.projections) {
                ...
                Evidence b;
                b.id = key; b.kind = "observation"; b.source = id; b.learned_at = e.time; b.roots = {e.root};
                b.statement = {e.participants.empty() ? id : e.participants.front(), "event", p, true, e.time, e.time};
                remember(id, b);
            }
        }
    }
    ...
    void World::read_message(const Id & reader, const Id & id) {
        ...
        Evidence e;
        e.id = id + "/read/" + reader; e.kind = "claim"; e.learned_at = state_.time + 1; e.source = m.author;
        e.roots = m.roots.empty() ? std::vector<Id>{id} : m.roots;
        e.confidence = .7;
        if (text(m.content, "type") == "claim" && has(m.content, "statement")) {
            ...
            e.statement = {m.author, text(m.content, "type", "message"), m.content, true, m.sent, m.sent};
            e.confidence = 1;
        }
        if (text(m.content, "type") == "proposal_update") {
            const auto answer = text(m.content, "answer");
            if (reader == text(m.content, "author") &&(answer == "accept" || answer == "decline")) ++ state_.actors.at(reader).outcome_counts[m.author + "/" + str(at(m.content, "terms"), "kind") + "/" +(answer == "accept" ? "accepted" : "declined")];
        }
        if (text(m.content, "type") == "demand" && get_or<bool>(m.content, "forceful", false)) appraise(reader, m.author, "demand/" + m.author + "/" + reader + "/" + text(m.content, "episode"), "demand");
        if (text(m.content, "type") == "claim" && truth_value(state_.actors.at(reader), e.statement, state_.time + 1) == "false") appraise(reader, m.author, e.roots.front(), "false_statement");
        remember(reader, e);
    }
    void World::send_message(const Action & a, const Json & content, const std::vector < Id > & recipients, const std::vector < Id > & roots) {
        Message m;
        m.id = a.id + "/message"; m.author = a.actor; m.recipients = recipients; m.content = content; m.roots = roots;
        m.channel = text(a.args, "channel", "speech"); m.sent = state_.time + 1; m.deliver_at = m.sent +(m.channel == "digital" ? 1 : 0);
        if (m.channel == "speech") m.recipients.erase(std::remove_if(m.recipients.begin(), m.recipients.end(),[&](const Id & id) { return ! channel_available(a.actor, id, "speech"); }), m.recipients.end());
        state_.messages.emplace(m.id, m);
        emit(a.id, "message_sent", {a.actor}, {}, Object{{"message", m.id}, {"channel", m.channel}}, true, a.proposal);
    }
    void World::appraise(const Id & observer, const Id & target, const Id & root, const std::string & type) {
        ...
        Appraisal a; a.observer = observer; a.target = target; a.root = root; a.type = type;
        ...
        if (type == "breach") a.anger_delta = config_.number({"stimuli", "other_promise_breach", "anger"});
        if (type == "demand") a.anger_delta = config_.number({"stimuli", "new_demand_episode", "anger_gap_trust_multiplier"}) *(1 - trust);
    ...
    void World::reactions() {
        ...
        for (auto a : pending_appraisals_) {
            ...
            if (a.type == "help" || a.type == "return") {
                ...
                if (! state_.appraisal_history.contains(key) && it != state_.positive_cooldowns.end() && state_.time + 1 - it->second < config_.number({"relationships", "positive_pair_cooldown_minutes"})) continue;
            ...
            if (auto it = state_.appraisal_history.find(key); it != state_.appraisal_history.end()) {
                ...
                delta.trust_delta -= it->second.trust_delta; delta.tension_delta -= it->second.tension_delta;
                delta.stress_delta -= it->second.stress_delta; delta.anger_delta -= it->second.anger_delta; delta.esteem_delta -= it->second.esteem_delta;
            }
            state_.appraisal_history[key] = a;
            auto & sum = effects[a.observer + "|" + a.target];
            sum.observer = a.observer; sum.target = a.target; sum.trust_delta += delta.trust_delta; sum.tension_delta += delta.tension_delta;
            sum.stress_delta += delta.stress_delta; sum.anger_delta += delta.anger_delta; sum.esteem_delta += delta.esteem_delta;
        }
        std::map<Id,std::pair<double,double>> emotional;
        for (const auto &[key, a] : effects) {
            auto & x = state_.actors.at(a.observer);
            if (! a.target.empty()) {
                auto & r = x.relations[a.target]; r.trust = math::clip(r.trust + a.trust_delta); r.tension = math::clip(r.tension + a.tension_delta);
                x.anger[a.target] = math::clip(x.anger[a.target] + a.anger_delta);
            }
            emotional[a.observer].first += a.stress_delta; emotional[a.observer].second += a.esteem_delta;
        }
        for (auto &[id, x] : state_.actors) {
            x.stress = math::clip(x.stress + emotional[id].first); x.selfesteem = math::clip(x.selfesteem + emotional[id].second);
            double fear = 0;
            for (const auto & b : x.beliefs) if (b.kind == "hypothesis" && b.statement.predicate == "threat" &&(b.statement.valid_until < 0 || b.statement.valid_until >= state_.time + 1)) {
                double p = b.half_life > 0 ? math::stale(b.confidence, b.prior, static_cast<double>(state_.time + 1 - b.learned_at), b.half_life) : b.confidence;
                fear = std::max(fear, p * b.severity);
            }
            x.fear = math::clip(fear);
            x.wellbeing = math::ema(x.wellbeing, static_cast<double>(state_.accounts.at(x.account)), config_.number({"money", "wellbeing_half_life_minutes"}));
        }
    }
    Json World::view(const Id & id) const {
        ...
        Json self = encode(a); self.as_object().erase("inbox");
        Object inventory;
        for (const auto &[key, i] : state_.items) if (holder(key) == id && accessible(id, key)) inventory[key] = Object{{"id", key}, {"type", i.type}, {"owner", i.owner}, {"condition", i.condition}, {"remaining_units", i.remaining_units}, {"version", i.version}};
        Array inbox;
        for (const auto & mid : a.inbox) {
            const auto & m = state_.messages.at(mid);
            Object entry{{"id", mid}, {"from", m.author}, {"channel", m.channel}, {"read", m.readers.contains(id)}};
            if (m.readers.contains(id)) entry["content"] = m.content;
            inbox.push_back(entry);
        }
        Array receipts;
        for (const auto &[key, r] : state_.receipts) if (r.actor == id) receipts.push_back(Object{{"id", key}, {"status", r.status}, {"action", r.action}, {"reason", r.public_reason}, {"received", r.received}, {"request", r.request}});
        Array jobs; for (const auto &[key, j] : state_.jobs) if (j.creator == id) jobs.push_back(encode(j));
        Array employment; for (const auto &[key, c] : state_.contracts) if (c.worker == id) employment.push_back(encode(c));
        return Object{{"own_jobs", jobs}, {"employment", employment}, {"version", std::string(rules_version)}, {"time", state_.time}, {"self", self}, {"money", state_.accounts.at(a.account)}, {"inventory", inventory}, {"inbox", inbox}, {"receipts", receipts}};
    }
}
